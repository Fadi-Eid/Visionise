...
with mp_face_detection.FaceDetection( model_selection=1,
  ...
  while True:
    ...
    #   print(results.detections[0].location_data.relative_keypoints[2].x) #print x of the nose
    #   for detection in results.detections:
    #     mp_drawing.draw_detection(image, detection)
    #     print(detection.location_data.relative_keypoints[2].x)
    else:
        new_x = 0.50

    print(new_x)

    if(0.46<new_x and new_x<0.54):
        xPos = 'm'+'\r'
    elif(new_x>=0.54):
        xPos = 'l'+'\r'
    elif(new_x<=0.46):
